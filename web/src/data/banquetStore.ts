...
export function logFollowUp(input: LogFollowUpInput): FollowUpLogEntry {
  ...
  if (!isOpenEnquiryStatus(enquiry.status)) throw new Error("Cannot follow up on a booked enquiry");
...
export function getOpenEnquiries(): EnquiryLogEntry[] {
  return store.enquiries.filter(
    (enquiry) => isOpenEnquiryStatus(enquiry.status) && !store.bookings.some((b) => b.enquiryId === enquiry.id),
  );
}
