...
export type CalendarEvent = {
  ...
  venue?: string;
  category: EventCategory;
  status: "confirmed" | "tentative" | "enquiry" | "lost";
...
export function getFollowUpStats(records = toFollowUpEnquiryRecords(), today = new Date()) {
  const openRecords = records.filter((record) => isOpenEnquiryStatus(record.status) && !record.bookingId);
  const open = openRecords.length;
  const overdue = openRecords.filter((record) => isFollowUpOverdue(record.nextFollowUpDate, today)).length;
  const dueThisWeek = openRecords.filter((record) => isFollowUpDueThisWeek(record.nextFollowUpDate, today)).length;
