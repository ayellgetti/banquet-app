...
import { fetchEnquiriesFromApi } from "@/lib/enquiriesApi";
...
type ApiFollowup = {
  ...
  enquiryStatus?: BackendLeadStatus;
  createdAt: string;
  enquiry: {
    id: string;
...
function mapFollowupToHistoryRecord(followup: ApiFollowup, status?: EnquiryStatus): FollowUpHistoryRecord {
  const enquiryStatus =
    status ?? mapLeadStatusToFrontend(followup.enquiryStatus ?? followup.enquiry.status);
