...
import { isUnconvertibleLeadStatus } from '../leads/lead-status.js';
import { cancelTentativeEventForClosedLead } from '../leads/close-lead-event.js';
...
export class FollowupService {
  ...
  async create(input: CreateFollowupInput): Promise<FollowupResponse> {
    ...
    const snapshotStatus = input.enquiryStatus ?? enquiry.status;
    ...
    if (input.enquiryStatus) {
      ...
      if (isUnconvertibleLeadStatus(enquiry.status) && input.enquiryStatus !== enquiry.status) {
        throw new AppError('Closed enquiries cannot be updated', 400);
      }

      if (input.enquiryStatus === LeadStatus.CONVERTED) {
        throw new AppError('Use the convert endpoint to mark an enquiry as converted', 400);
      }
      ...
      if (enquiry.status !== input.enquiryStatus) {
        ...
        await cancelTentativeEventForClosedLead(this.prisma, enquiryId, input.enquiryStatus);
      }
    }
