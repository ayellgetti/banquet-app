...
export class EnquiryService {
  ...
  async update(id: bigint, input: UpdateEnquiryInput): Promise<EnquiryResponse> {
    ...
    if (input.status) {
      await cancelTentativeEventForClosedLead(this.prisma, id, input.status);
    }
  ...
  async convert(id: bigint, input: ConvertEnquiryInput): Promise<ConvertEnquiryResult> {
    ...
    if (isUnconvertibleLeadStatus(enquiry.status)) {
      throw new AppError('Closed enquiries cannot be converted', 400);
    }
