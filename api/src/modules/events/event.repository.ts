...
import { UNCONVERTIBLE_LEAD_STATUSES } from '../leads/lead-status.js';
...
export class EventRepository {
  ...
  private buildWhere(
    ...
    options?: { activeCalendar?: boolean },
  ): Prisma.EventWhereInput | undefined {
    const conditions: Prisma.EventWhereInput[] = [];

    if (options?.activeCalendar && !filters.status) {
      // Keep lost leads visible on the calendar (as "lost"), but drop
      // events that were genuinely cancelled while the lead was still active.
      conditions.push({
        NOT: {
          status: EventStatus.CANCELLED,
          enquiry: { status: { notIn: UNCONVERTIBLE_LEAD_STATUSES } },
        },
      });
    }
